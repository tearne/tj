...
- **Overlay markers in the buffer pipeline**: Beat tick marks and the cue point line are computed by the background thread at the same time as the waveform, stored in the buffer using the same anchor and `samples_per_col`. The draw thread maps them to screen coordinates via the same `viewport_start` transform as the waveform, guaranteeing they cannot drift relative to it. Tick marks are stored as semantic sub-column values (`0x47` = left half-column, `0xB8` = right half-column, `0` = none) and extracted with a semantic half-column shift. The cue point is stored as a single buffer column index (`cue_buf_col: Option<usize>`) and mapped directly to a screen column via `viewport_start`.
...
**Buffer space**
The coordinate of the pre-rendered braille byte buffer: an array of cells indexed 0 to `buf_cols − 1`, each corresponding to one column-grid cell. The buffer is wider than the screen and centred on the **anchor** — the column-grid cell nearest the current playhead position. Each buffer holds the waveform grid (raw braille bit patterns), a tick row (semantic sub-column values), and the cue point column index. Tick marks are stored semantically rather than as raw braille bits so they can be correctly transformed through the half-column viewport shift without scrambling. The cue point is stored as a single column index (`Option<usize>`) and needs no sub-column handling since it replaces the full character at that column.

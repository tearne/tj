...
- When no tempo adjustment is active, the native BPM (`base_bpm`) is shown to two decimal places (e.g. `120.00`) and receives a soft amber beat-flash. When a per-deck BPM adjustment is active, the native BPM is shown plain to two decimal places and the playback BPM is shown alongside in parentheses to one decimal place (e.g. `120.00 (124.4)`), with only the playback BPM receiving the beat-flash. The two precisions reflect the adjustment resolutions: `X`/`S` adjust `base_bpm` in 0.01 steps; `x`/`s` adjust playback BPM in 0.1 steps. All underlying values retain full precision.
- **Vinyl mode** — When vinyl mode is active, the BPM field, beat-flash, phase offset, and metronome indicator are replaced by a playback speed display showing the current speed as a percentage of the track's nominal rate (e.g. `+0.3%`, `-1.2%`, `0.0%`). The percentage is derived from `vinyl_speed` (1.0 = nominal). The nudge mode indicator, level, and spectrum strip are unchanged.
- **Beat mode without established BPM** — When a deck in beat mode has no established BPM, the BPM field is replaced by the same percentage speed display (derived from `bpm / base_bpm`). Phase offset and metronome indicator remain visible but dormant. Once a BPM is established the display reverts to the normal BPM field.

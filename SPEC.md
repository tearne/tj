...
- The user can correct an inaccurate detection at runtime:
  - `v` increases the effective BPM by 0.01; `f` decreases it by 0.01. Adjustments affect playback speed proportionally (relative to the detected BPM) and clamp to the range 40.0–240.0.
...
- `V` increases `base_bpm` by 0.01; `F` decreases it by 0.01. Both clamp to 40.0–240.0. Adjusting `base_bpm` resets any active `f`/`v` playback offset (`bpm` is set equal to the new `base_bpm`, speed returns to 1×) and is persisted to the cache immediately.
